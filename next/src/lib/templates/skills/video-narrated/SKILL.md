...
5. **渲染**: playwright 无头逐帧抓取动画 → ffmpeg 合成视频; 若有配音, 用 ffmpeg 把配音轨混入 (`-map` 视频流 + 音频流, `-shortest`) → `out/<slug>.mp4`。若用户提供了 BGM, 把它垫在配音之下 (BGM 压到 -20dB 以下, 配音为主); 无 BGM 跳过。**不要**下载版权不清的音频。
6. 清理临时帧/脚本/音频中间件。

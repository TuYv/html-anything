...
【步骤】
1. 按【用户内容】生成一个 1920×1080 单文件动画 HTML (CSS 时间线; 真实内容; 遵守设计纪律)。转场风格统一 (fade/slide/wipe/mask, 单次 0.3-0.6s, 不每屏换花样); 渲染后可用 ffmpeg 统一调色 (eq/curves/LUT) 给整片定调。
2. 写逐场景**解说脚本** (默认中文; 若【用户内容】指定了语言/语种, 用该语言写脚本, 并让 TTS 用对应语言的音色; 节奏与各场景时长匹配)。
...
5. **渲染**: playwright 无头逐帧抓取动画 → ffmpeg 合成视频; 若有配音, 用 ffmpeg 把配音轨混入 (`-map` 视频流 + 音频流, `-shortest`) → `out/<slug>.mp4`。BGM 可选: 内置曲库在 `$HTML_ANYTHING_AUDIO_DIR/bgm/` (6 首) —— 选一首垫在配音之下 (压到 -20dB 以下, 配音为主), 或用用户自带; 无则跳过。`$HTML_ANYTHING_AUDIO_DIR/sfx/` 有分类音效可点缀。**不要**下载版权不清的音频。
6. 清理临时帧/脚本/音频中间件。
